...
class PlayerService:
    ...
    async def resolve_player(self, game_name: str, tag_line: str):
        ...
        if not account:
            return None
        await self.repo.upsert_player(account.puuid, account.game_name, account.tag_line)
        await self.session.commit()
        return await self.repo.get_profile(account.puuid)

    async def refresh_player(self, game_name: str, tag_line: str, match_depth: int = 20):
        account = await self.riot.get_account_by_riot_id(game_name, tag_line)
        if not account:
            return None
        await self.repo.upsert_player(account.puuid, account.game_name, account.tag_line)
        ...
        match_ids = await self.riot.get_matches_by_puuid(account.puuid, count=match_depth)
    ...
    async def get_champion_aggregates(self, game_name: str, tag_line: str):
        ...
        for r in rows:
            ...
            data.append(ChampionAggregateResponse(
